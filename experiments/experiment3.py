...
def single_dataset(beta_true: np.array,
                   n: int,
                   rho: float,
                   glm: GLM,
    ...
    results.append({'dataset': dataset + 1,
                    ...
                    'time': end - start})
    print(f"LA results are ready. Time elapsed: {end - start}")
    print("Starting", smc_runs, "tries of ALASMC.")
    for smc_run in range(smc_runs):
        ...
        results.append({'dataset': dataset + 1,
                        ...
                        'time': end - start})
        print(f"ALASMC done! [{smc_run + 1} / {smc_runs}]. Elapsed time: {end - start}.")
    return results


def multiple_datasets(beta_true: np.array,
                      n: int,
                      rho: float,
                      glm: GLM,
                      optimization_procedure: object,
    ...
    for dataset in range(n_datasets):
        results = results + single_dataset(beta_true=beta_true,
                                           n=n,
                                           rho=rho,
                                           glm=glm,
                                           optimization_procedure=optimization_procedure,
...
if __name__ == "__main__":
    ...
    result = []
    param_grid = np.array(np.meshgrid(p_list, glm_list)).T.reshape(-1, 2)

    for p, glm in param_grid:
        ...
        beta_true = np.append(np.zeros(p - n_active), np.repeat(0.5, n_active))
        result = result + multiple_datasets(beta_true, n, rho, glm, NewtonRaphson(), normal_prior_log,
                                            beta_binomial_prior_log, SimpleGibbsKernel, burnin, particle_number,
                                            smc_runs, n_datasets, adjusted_curvature, False, tol_grad,
                                            tol_loglike, False)
    with open(f'results/experiment3_new.json', 'w') as file:
        json.dump(result, file)
    print(f"The Experiment 3 is finished.")
